...
async function main() {
    ...
    headers.push(head[8])
    headers.push(head[9])
    headers.push(head[10])
    headers.push(head[11])
    headers.push(head[12])
//HEADERS END
...
let data51 ={SYMBOL:ADANIENT[0],OPEN:ADANIENT[1],HIGH:ADANIENT[2],LOW:ADANIENT[3],PREVCLOSE:ADANIENT[4],LTP:ADANIENT[5],CHNG:ADANIENT[6],CHANGPercentage:ADANIENT[7],VOLUME:ADANIENT[8],VALUE:ADANIENT[9],fiftyTwo_WH:ADANIENT[10],fiftyTwo_WL:ADANIENT[11],TODAY:ADANIENT[12],Date:moment().format('DD-MM-YY'),Time:moment().format('hh.mm')}
    ...
    url: 'https://api.niftyleveltracker.in/App/api/v1/scrapData',
    headers: { 
    ...
    .catch(async(error) => {
        await browser.close();
        main();
    console.log('Internet Speed Is Slow',error.response.status);
    });
    // console.log("550: Post request");
    console.log('551: Stocks Post Successfully');
    ...
    try {
        ...
        console.log('ERROR Somthing Went Wrong Plase check Your Internet Connection Or restart App By Simply Typing rs and hit Enter ');
    }
}
...
let config = {
    method: 'get',
    maxBodyLength: Infinity,
    url: 'https://api.niftyleveltracker.in/App/api/v1/getScrapData/',
...
.catch((error) => {
    console.log('606:',error.response.data.message);
});
  //for Sheet 3//
...
let configA = {
    method: 'get',
    maxBodyLength: Infinity,
    url: 'https://api.niftyleveltracker.in/App/api/v1/getScrapData20',
...
.then((response) => {
    console.log(`622: Get request for sheet 3 with time ${time1}`);
    covert(response.data,2)
})
.catch((error) => {
    console.log('626:',error.response.data.message);
});
...
function covert(data,funcNu){
    try {
        ...
        let GetStock = []
        console.log("629: Data converted >>>")
        ...
        console.log('ERROR While Convrting Data ');
    }
} 
 //insert into sheet 2
...
console.log("703: >>>> Sheet 2 updated",res1.status);  
} catch (error) {
    console.log('ERROR While Updating Sheet 2');
}
}
...
try {
    ...
    let res1 = await gsApi.spreadsheets.values.update(updateOption1); // post
    console.log("719: >>>> Sheet 3 updated",res1.status);
} catch (error) {
    console.log('ERROR While Updating Sheet 3');
}
